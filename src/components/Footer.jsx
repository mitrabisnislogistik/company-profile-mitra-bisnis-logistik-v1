function Footer() {
	return (
		<div className="lg:m-8 lg:p-8 mb-0 pb-0 mulish">
			<hr className="border-2 border-blue-900 rounded-full" />
			<div className="m-8 px-8 grid lg:grid-cols-2 grid-cols-1">
				<div className="flex items-center mb-8">
					<img
						src="https://ik.imagekit.io/5mtwtegkx/mitra-bisnis-logistik-assets/pt%20mitra%20bisnis%20logistik.png?updatedAt=1750859192046"
						alt="logo-footer"
						className="max-h-12 me-4"
					/>
					<p className="pt-serif font-semibold text-true-blue">
						PT. Mitra Bisnis Logistik
					</p>
				</div>
				<div className="grid grid-rows-3 gap-4">
					<div className="flex items-center">
						<svg className="me-4 max-w-12" viewBox="-2 0 32 32">
							<g
								id="Page-1"
								stroke="none"
								stroke-width="1"
								fill="none"
								fill-rule="evenodd">
								<g
									transform="translate(-106.000000, -413.000000)"
									fill="#1d1e92">
									<path d="M118,422 C116.343,422 115,423.343 115,425 C115,426.657 116.343,428 118,428 C119.657,428 121,426.657 121,425 C121,423.343 119.657,422 118,422 L118,422 Z M118,430 C115.239,430 113,427.762 113,425 C113,422.238 115.239,420 118,420 C120.761,420 123,422.238 123,425 C123,427.762 120.761,430 118,430 L118,430 Z M118,413 C111.373,413 106,418.373 106,425 C106,430.018 116.005,445.011 118,445 C119.964,445.011 130,429.95 130,425 C130,418.373 124.627,413 118,413 L118,413 Z"></path>
								</g>
							</g>
						</svg>
						<p>
							Jl. Teluk Naga - Desa Teluk Naga Kec. Teluk, Naga, Kab. Tangerang,
							Banten 15510
						</p>
					</div>
					<div className="flex items-center">
						<svg
							className="me-4"
							width="38px"
							height="38px"
							viewBox="0 0 20 20">
							<g
								id="Page-1"
								stroke="none"
								stroke-width="1"
								fill="none"
								fill-rule="evenodd">
								<g
									transform="translate(-300.000000, -7599.000000)"
									fill="#1d1e92">
									<g transform="translate(56.000000, 160.000000)">
										<path d="M259.821,7453.12124 C259.58,7453.80344 258.622,7454.36761 257.858,7454.53266 C257.335,7454.64369 256.653,7454.73172 254.355,7453.77943 C251.774,7452.71011 248.19,7448.90097 248.19,7446.36621 C248.19,7445.07582 248.934,7443.57337 250.235,7443.57337 C250.861,7443.57337 250.999,7443.58538 251.205,7444.07952 C251.446,7444.6617 252.034,7446.09613 252.104,7446.24317 C252.393,7446.84635 251.81,7447.19946 251.387,7447.72462 C251.252,7447.88266 251.099,7448.05372 251.27,7448.3478 C251.44,7448.63589 252.028,7449.59418 252.892,7450.36341 C254.008,7451.35771 254.913,7451.6748 255.237,7451.80984 C255.478,7451.90987 255.766,7451.88687 255.942,7451.69881 C256.165,7451.45774 256.442,7451.05762 256.724,7450.6635 C256.923,7450.38141 257.176,7450.3464 257.441,7450.44643 C257.62,7450.50845 259.895,7451.56477 259.991,7451.73382 C260.062,7451.85686 260.062,7452.43903 259.821,7453.12124 M254.002,7439 L253.997,7439 L253.997,7439 C248.484,7439 244,7443.48535 244,7449 C244,7451.18666 244.705,7453.21526 245.904,7454.86076 L244.658,7458.57687 L248.501,7457.3485 C250.082,7458.39482 251.969,7459 254.002,7459 C259.515,7459 264,7454.51465 264,7449 C264,7443.48535 259.515,7439 254.002,7439"></path>
									</g>
								</g>
							</g>
						</svg>
						<p>+62 812 9262 9298</p>
					</div>
					<div className="flex items-center">
						<svg
							className="me-4"
							fill="#1d1e92"
							width="38px"
							height="38px"
							viewBox="0 0 24 24">
							<path d="M22,5V9L12,13,2,9V5A1,1,0,0,1,3,4H21A1,1,0,0,1,22,5ZM2,11.154V19a1,1,0,0,0,1,1H21a1,1,0,0,0,1-1V11.154l-10,4Z" />
						</svg>
						<p>mitrabisnislogistik@gmail.com</p>
					</div>
				</div>
			</div>
		</div>
	);
}

export default Footer;
